...
function ajax_form(){
    ...
    if(!judge_validate(4,'分类名称(按人数)',person_name,true,'length',1,20)){
        return false;
    }
    ...
    var prefix_id = $('input[name=prefix_id]:checked').val() || '';
    var judge_seled = judge_validate(1,'排号前缀',prefix_id,true,'digit','',"");
    if(judge_seled != ''){
        layer_alert("请选择排号前缀",3,0);
        //err_alert('<font color="#000000">' + judge_seled + '</font>');
        return false;
    }
