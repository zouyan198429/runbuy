...
function initList(){
    // 获得选中的城市id 数组
    var SELECTED_IDS = parent.getSelectedSellerIds();
...
(function() {
   ...
   // document.write("        if( item.issuper==0 ){");
    ...
    document.write("            <td><%=item.city_site_name%><hr/><%=item.partner_name%><\/td>");
    document.write("            <td><%=item.seller_name%><hr/><%=item.linkman%><\/td>");
