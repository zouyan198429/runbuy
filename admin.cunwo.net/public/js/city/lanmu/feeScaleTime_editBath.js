...
function ajax_form(){
    ...
    var price_distance_every = $('input[name=price_distance_every]').val();
    if( !judge_validate(4,'超出2公里，每公里费用',price_distance_every,true,'doublepositive','','')){
        return false;
    }

    var price_shop_every = $('input[name=price_shop_every]').val();
    if( !judge_validate(4,'每多跑一商家费用',price_shop_every,true,'doublepositive','','')){
        return false;
    }

    // 判断时间段价格
    var timePriceErr = false;
    $('.time_list').find('tr').each(function () {
        var trObj = $(this);
        var  timeName = trObj.data('time_name');

        var init_price_val = trObj.find('input[name="init_prices[]"]').val();
        if( !judge_validate(4,'时间段[' + timeName + ']价格',init_price_val,true,'doublepositive','','')){
            timePriceErr = true;
            return false;
        }
    });
    if( timePriceErr ) return false;
