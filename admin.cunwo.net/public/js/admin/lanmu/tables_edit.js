...
const REL_CHANGE = {
    'tablePerson':{// 店铺桌位分类-二级分类
        'child_sel_name': 'table_person_id',// 第二级下拉框的name
        'child_sel_txt': {'': "请选择桌位分类" },// 第二级下拉框的{值:请选择文字名称}
        'change_ajax_url': TABLE_PERSON_KV_URL,// 获取下级的ajax地址
...
$(function(){
    //当前店铺桌位分类
    if(SHOP_ID > 0){
        changeFirstSel(REL_CHANGE.tablePerson,SHOP_ID,TABLE_PERSON_ID, true);
    }
    ...
    $(document).on("change",'input[name=shop_id]',function(){
        ...
        var tem_config = REL_CHANGE.tablePerson;
...
function ajax_form(){
    ...
    var table_person_id = $('select[name=table_person_id]').val();
    var judge_seled = judge_validate(1,'分类',table_person_id,true,'digit','','');
    if(judge_seled != ''){
    ...
    // var table_person_id = $('input[name=table_person_id]:checked').val() || '';
    // var judge_seled = judge_validate(1,'分类(按人数)',table_person_id,true,'digit','',"");
    // if(judge_seled != ''){
    //     layer_alert("请选择分类(按人数)",3,0);
    //     //err_alert('<font color="#000000">' + judge_seled + '</font>');
    //     return false;
    // }

    var table_name = $('input[name=table_name]').val();
    if(!judge_validate(4,'桌位号/包间名称',table_name,true,'length',1,20)){
        return false;
    }

    var is_open = $('input[name=is_open]:checked').val() || '';
    // var judge_seled = judge_validate(1,'是否开启',is_open,true,'custom',/^[012]$/,"");
    var judge_seled = judge_validate(1,'是否开启',is_open,true,'digit','',"");
    if(judge_seled != ''){
        layer_alert("请选择是否开启",3,0);
        //err_alert('<font color="#000000">' + judge_seled + '</font>');
        return false;
    }
