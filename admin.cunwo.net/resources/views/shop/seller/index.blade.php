    ...
    <form onsubmit="return false;" class="form-horizontal" role="form" method="post" id="search_frm" action="#">
      <div class="msearch fr" style="width:700px;">
        ...
        <select style="width:80px; height:28px;" name="field">
          ...
          <option value="city_site_id">城市分站id</option>
          <option value="city_partner_id">城市合伙人id</option>
        </select>
...
--}}
  ...
  <script type="text/javascript">
      ...
      var AJAX_URL = "{{ url('api/shop/seller/ajax_alist') }}";//ajax请求的url
      var ADD_URL = "{{ url('shop/seller/add/0') }}"; //添加url

      var IFRAME_MODIFY_URL = "{{url('shop/seller/add/')}}/";//添加/修改页面地址前缀 + id
      var IFRAME_MODIFY_URL_TITLE = "商家" ;// 详情弹窗显示提示  [添加/修改] +  栏目/主题
      var IFRAME_MODIFY_CLOSE_OPERATE = 0 ;// 详情弹窗operate_num关闭时的操作0不做任何操作1刷新当前页面2刷新当前列表页面

      var SHOW_URL = "{{url('shop/seller/info/')}}/";//显示页面地址前缀 + id
      var SHOW_URL_TITLE = "" ;// 详情弹窗显示提示
      var SHOW_CLOSE_OPERATE = 0 ;// 详情弹窗operate_num关闭时的操作0不做任何操作1刷新当前页面2刷新当前列表页面
      var EDIT_URL = "{{url('shop/seller/add/')}}/";//修改页面地址前缀 + id
      var DEL_URL = "{{ url('api/shop/seller/ajax_del') }}";//删除页面地址
      var BATCH_DEL_URL = "{{ url('api/shop/seller/ajax_del') }}";//批量删除页面地址
      var EXPORT_EXCEL_URL = "{{ url('shop/seller/export') }}";//导出EXCEL地址
      var IMPORT_EXCEL_TEMPLATE_URL = "{{ url('shop/seller/import_template') }}";//导入EXCEL模版地址
      var IMPORT_EXCEL_URL = "{{ url('api/shop/seller/import') }}";//导入EXCEL地址
      ...
      var STAFF_SELLER_LIST_URL = "{{ url('shop/staffSeller') }}"; //帐号管理
      var SHOP_LIST_URL = "{{ url('shop/shop') }}"; // 店铺管理
  ...
  <script src="{{ asset('js/shop/lanmu/seller.js') }}"  type="text/javascript"></script>
</body>
</html>