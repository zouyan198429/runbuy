...
<div class="mm">
  <div class="mmhead" id="mywork">
    ...
    <div class="tabbox" >
      <a href="javascript:void(0);" class="on" onclick="action.iframeModify(0)">添加城市代理</a>
    </div>
    <form onsubmit="return false;" class="form-horizontal" role="form" method="post" id="search_frm" action="#">
      <div class="msearch fr" style="width:700px;">
        ...
        <select style="width:80px; height:28px;" name="field">
          <option value="partner_name">名称</option>
...
--}}
  ...
  <script type="text/javascript">
      ...
      var AJAX_URL = "{{ url('api/shop/cityPartner/ajax_alist') }}";//ajax请求的url
      var ADD_URL = "{{ url('shop/cityPartner/add/0') }}"; //添加url

      var IFRAME_MODIFY_URL = "{{url('shop/cityPartner/add/')}}/";//添加/修改页面地址前缀 + id
      var IFRAME_MODIFY_URL_TITLE = "城市代理" ;// 详情弹窗显示提示  [添加/修改] +  栏目/主题
      var IFRAME_MODIFY_CLOSE_OPERATE = 0 ;// 详情弹窗operate_num关闭时的操作0不做任何操作1刷新当前页面2刷新当前列表页面

      var SHOW_URL = "{{url('shop/cityPartner/info/')}}/";//显示页面地址前缀 + id
      var SHOW_URL_TITLE = "" ;// 详情弹窗显示提示
      var SHOW_CLOSE_OPERATE = 0 ;// 详情弹窗operate_num关闭时的操作0不做任何操作1刷新当前页面2刷新当前列表页面
      var EDIT_URL = "{{url('shop/cityPartner/add/')}}/";//修改页面地址前缀 + id
      var DEL_URL = "{{ url('api/shop/cityPartner/ajax_del') }}";//删除页面地址
      var BATCH_DEL_URL = "{{ url('api/shop/cityPartner/ajax_del') }}";//批量删除页面地址
      var EXPORT_EXCEL_URL = "{{ url('shop/cityPartner/export') }}";//导出EXCEL地址
      var IMPORT_EXCEL_TEMPLATE_URL = "{{ url('shop/cityPartner/import_template') }}";//导入EXCEL模版地址
      var IMPORT_EXCEL_URL = "{{ url('api/shop/cityPartner/import') }}";//导入EXCEL地址
      ...
      var STAFF_PARTNER_LIST_URL = "{{ url('shop/staffPartner') }}"; //帐号管理
      var SELLER_LIST_URL = "{{ url('shop/seller') }}"; // 商家管理
      var STAFF_RUN_LIST_URL = "{{ url('shop/staffRun') }}"; //跑腿人员管理
      var STAFF_USER_LIST_URL = "{{ url('shop/staffUser') }}"; //用户管理
      var ORDERS_LIST_URL = "{{ url('shop/order') }}"; // 订单管理
  </script>
  <script src="{{asset('js/common/list.js')}}"></script>
  <script src="{{ asset('js/shop/lanmu/cityPartner.js') }}"  type="text/javascript"></script>
</body>
</html>