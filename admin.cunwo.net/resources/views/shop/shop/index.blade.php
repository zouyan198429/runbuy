...
<!--       <th>
  ...
  <script type="text/javascript">
      ...
      var AJAX_URL = "{{ url('api/shop/shop/ajax_alist') }}";//ajax请求的url
      var ADD_URL = "{{ url('shop/shop/add/0') }}"; //添加url

      var IFRAME_MODIFY_URL = "{{url('shop/shop/add/')}}/";//添加/修改页面地址前缀 + id
      var IFRAME_MODIFY_URL_TITLE = "店铺" ;// 详情弹窗显示提示  [添加/修改] +  栏目/主题
      var IFRAME_MODIFY_CLOSE_OPERATE = 0 ;// 详情弹窗operate_num关闭时的操作0不做任何操作1刷新当前页面2刷新当前列表页面

      var SHOW_URL = "{{url('shop/shop/info/')}}/";//显示页面地址前缀 + id
      var SHOW_URL_TITLE = "" ;// 详情弹窗显示提示
      var SHOW_CLOSE_OPERATE = 0 ;// 详情弹窗operate_num关闭时的操作0不做任何操作1刷新当前页面2刷新当前列表页面
      var EDIT_URL = "{{url('shop/shop/add/')}}/";//修改页面地址前缀 + id
      var DEL_URL = "{{ url('api/shop/shop/ajax_del') }}";//删除页面地址
      var BATCH_DEL_URL = "{{ url('api/shop/shop/ajax_del') }}";//批量删除页面地址
      var EXPORT_EXCEL_URL = "{{ url('shop/shop/export') }}";//导出EXCEL地址
      var IMPORT_EXCEL_TEMPLATE_URL = "{{ url('shop/shop/import_template') }}";//导入EXCEL模版地址
      var IMPORT_EXCEL_URL = "{{ url('api/shop/shop/import') }}";//导入EXCEL地址
      ...
      var STAFF_SHOP_LIST_URL = "{{ url('shop/staffShop') }}"; //帐号管理
      var GOODS_TYPE_LIST_URL = "{{ url('shop/shopGoodsType') }}"; // 商品分类管理
      var PROP_LIST_URL = "{{ url('shop/prop') }}"; // 商品属性管理
      var GOODS_LIST_URL = "{{ url('shop/shopGoods') }}"; // 商品管理
      var ORDERS_LIST_URL = "{{ url('shop/order') }}"; // 订单管理
      var OPEN_TIME_LIST_URL = "{{ url('shop/shopOpenTime') }}"; // 营业时间管理

      var SAVE_URL_CLOSE = "{{ url('api/shop/shop/ajax_save_close') }}";// ajax保存记录-息业
      var SAVE_URL_OPEN = "{{ url('api/shop/shop/ajax_save_open') }}";// ajax保存记录-开业
  </script>
...
<script src="{{asset('js/common/list.js')}}"></script>
  <script src="{{ asset('js/shop/lanmu/shop.js') }}"  type="text/javascript"></script>
</body>
</html>