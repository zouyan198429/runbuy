...
<div style="display:none;">
  ...
  <script type="text/javascript">
      ...
      var AJAX_URL = "{{ url('api/city/order/ajax_alist') }}";//ajax请求的url
      var ADD_URL = "{{ url('city/order/add/0') }}"; //添加url

      var IFRAME_MODIFY_URL = "{{url('city/order/add/')}}/";//添加/修改页面地址前缀 + id
      var IFRAME_MODIFY_URL_TITLE = "用户" ;// 详情弹窗显示提示  [添加/修改] +  栏目/主题
      var IFRAME_MODIFY_CLOSE_OPERATE = 0 ;// 详情弹窗operate_num关闭时的操作0不做任何操作1刷新当前页面2刷新当前列表页面

      var SHOW_URL = "{{url('city/order/info/')}}/";//显示页面地址前缀 + id
      var SHOW_URL_TITLE = "" ;// 详情弹窗显示提示
      var SHOW_CLOSE_OPERATE = 0 ;// 详情弹窗operate_num关闭时的操作0不做任何操作1刷新当前页面2刷新当前列表页面
      var EDIT_URL = "{{url('city/order/add/')}}/";//修改页面地址前缀 + id
      var DEL_URL = "{{ url('api/city/order/ajax_del') }}";//删除页面地址
      var BATCH_DEL_URL = "{{ url('api/city/order/ajax_del') }}";//批量删除页面地址
      var EXPORT_EXCEL_URL = "{{ url('city/order/export') }}";//导出EXCEL地址
      var IMPORT_EXCEL_TEMPLATE_URL = "{{ url('city/order/import_template') }}";//导入EXCEL模版地址
      var IMPORT_EXCEL_URL = "{{ url('api/city/order/import') }}";//导入EXCEL地址
      ...
      var SATUS_COUNT_URL = "{{ url('api/city/order/ajax_status_count') }}";// ajax工单状态统计 url
      var NEED_PLAY_STATUS = "{{ $countPlayStatus }}";// 需要发声的状态，多个逗号,分隔

      var CANCEL_ORDER_URL = "{{ url('api/city/order/refundOrder') }}";// 取消订单
...
<script src="{{ asset('js/city/lanmu/orders.js') }}"  type="text/javascript"></script>
</body>
</html>