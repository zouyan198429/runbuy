...
class CTAPIFeeScaleTimeBusiness extends BasicPublicCTAPIBusiness
{
    public static $model_name = 'API\RunBuy\FeeScaleTimeAPI';
    public static $timeCityListDefault = [
        '1' => [
            'id' => 0,
            'time_num' => 1,
            'begin_time' => '00:00:00',
...
//                //['mobile', $keyword],
            ...
            'select' => [
                'id','city_site_id', 'time_num', 'begin_time','end_time','init_price', 'sort_num'
                ,'operate_staff_id','operate_staff_id_history'
                ,'created_at','updated_at'
            ],
            // 'orderBy' => ['sort_num'=>'desc', 'id'=>'desc'],//
            'orderBy' => ['time_num'=>'asc', 'id'=>'desc'],//
        ];// 查询条件参数
...
//        $result['data_list'] = $data_list;
     ...
     * @param boolean $modifAddOprate 修改时是否加操作人，true:加;false:不加[默认]
     * @param int $notLog 是否需要登陆 0需要1不需要
     * @return  int $city_site_id 返回城市id array 单条数据 - -维数组 为0 新加，返回新的对象数组[-维],  > 0 ：修改对应的记录，返回true
     * @author zouyan(305463219@qq.com)
     */
    public static function saveTimesByCityId(Request $request, Controller $controller, $saveData, &$city_site_id, $modifAddOprate = false, $notLog = 0)
    {
        $company_id = $controller->company_id;
        ...
        $city_site_id = static::exeDBBusinessMethodCT($request, $controller, '', 'saveTimesByCityId', $apiParams, $company_id, $notLog);
        return $city_site_id;
    }
