...
class CTAPISellerBusiness extends BasicPublicCTAPIBusiness
{
    public static $model_name = 'API\RunBuy\SellerAPI';
    public static $table_name = 'seller';// 表名称
...
//            ],
        ...
        foreach($data_list as $k => $v){
            ...
            // 城市分站名称
            $data_list[$k]['city_site_name'] = $v['seller_city']['city_name'] ?? '';
            // $data_list[$k]['city_site_id'] = $v['seller_city']['id'] ?? 0;
            if(isset($data_list[$k]['seller_city'])) unset($data_list[$k]['seller_city']);
            // 城市城市合伙人
            $data_list[$k]['partner_name'] = $v['seller_city_partner']['partner_name'] ?? '';
            // $data_list[$k]['city_partner_id'] = $v['seller_city_partner']['id'] ?? 0;
            if(isset($data_list[$k]['seller_city_partner'])) unset($data_list[$k]['seller_city_partner']);
        }
...
//        static::judgePowerByObj($request, $controller, $info, $judgeData );
        // 城市代理
        $partner_name = $info['seller_city_partner_history']['partner_name'] ?? '';
        if(empty($partner_name)) $partner_name = $info['seller_city_partner']['partner_name'] ?? '';
        $info['partner_name'] = $partner_name;
        $now_partner_state = 0;// 最新的城市代理 0没有变化 ;1 已经删除  2 试卷不同
        if(isset($info['seller_city_partner_history']) && isset($info['seller_city_partner'])){
            $history_version_num = $info['seller_city_partner_history']['version_num'] ?? '';
            $version_num = $info['seller_city_partner']['version_num'] ?? '';
            if(empty($info['seller_city_partner'])){
                $now_partner_state = 1;
            }elseif($version_num != '' && $history_version_num != $version_num){
                $now_partner_state = 2;
            }
        }
        if(isset($info['seller_city_partner_history'])) unset($info['seller_city_partner_history']);
        if(isset($info['seller_city_partner'])) unset($info['seller_city_partner']);
