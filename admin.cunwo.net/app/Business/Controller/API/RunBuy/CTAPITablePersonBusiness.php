...
class CTAPITablePersonBusiness extends BasicPublicCTAPIBusiness
{
    public static $model_name = 'API\RunBuy\TablePersonAPI';
...
//            if($province_id > 0 )  array_push($queryParams['where'], ['city_ids', 'like', '' . $province_id . ',%']);
            ...
            $prefix_id = CommonRequest::get($request, 'prefix_id');
            if(is_numeric($prefix_id) && $prefix_id >= 0 )  array_push($queryParams['where'], ['prefix_id', '=', $prefix_id]);
        ...
        foreach($data_list as $k => $v){
            ...
            // 排号前缀
            $data_list[$k]['prefix_name'] = $v['num_prefix']['prefix_name'] ?? '';
            // $data_list[$k]['shop_id'] = $v['shop']['id'] ?? 0;
            if(isset($data_list[$k]['num_prefix'])) unset($data_list[$k]['num_prefix']);
