...
use App\Business\API\RunBuy\CityPartnerHistoryAPIBusiness;
...
class CTAPICityPartnerBusiness extends BasicPublicCTAPIBusiness
{
    public static $model_name = 'API\RunBuy\CityPartnerAPI';
...
//            ],
        ...
        foreach($data_list as $k => $v){
            ...
            // 城市分站名称
            $data_list[$k]['city_site_name'] = $v['city_partner_city']['city_name'] ?? '';
            // $data_list[$k]['city_site_id'] = $v['city_partner_city']['id'] ?? 0;
            if(isset($data_list[$k]['city_partner_city'])) unset($data_list[$k]['city_partner_city']);
        }
...
//        static::judgePowerByObj($request, $controller, $info, $judgeData );
        // 城市分站
        $city_name = $info['city_partner_city_history']['city_name'] ?? '';
        if(empty($city_name)) $city_name = $info['city_partner_city']['city_name'] ?? '';
        $info['city_name'] = $city_name;
        $now_city_state = 0;// 最新的试题 0没有变化 ;1 已经删除  2 试卷不同
        if(isset($info['city_partner_city_history']) && isset($info['city_partner_city'])){
            $history_version_num = $info['city_partner_city_history']['version_num'] ?? '';
            $version_num = $info['city_partner_city']['version_num'] ?? '';
            if(empty($info['city_partner_city'])){
                $now_city_state = 1;
...
//                'city_partner_id' => $id,
//            ];
//            static::compareHistoryOrUpdateVersion($request, $controller, '' , $id, CityPartnerHistoryAPIBusiness::$model_name
//                , 'city_partner_history', $historySearch, ['city_partner_id'], 1, $company_id);
//        }
//        return $result;
    }
